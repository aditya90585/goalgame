...
const Navbar = () => {
  ...
  return (
    ...
    <nav className='md:bg-[#2D520F] md:w-full w-[99%] h-8 rounded-2xl flex justify-between items-center md:static absolute bottom-0'>
      <div className='flex w-1/3'>

        <div onClick={toggleGamesselector} className='bg-[#2F5300] cursor-pointer rounded-xl md:w-35 w-60 h-6 ml-1 text-white flex justify-center items-center active:translate-x-0.2 active:translate-y-0.5  transition-transform duration-150 inset-shadow-[0.8px_0.6px_0px_#536D23] shadow-[1px_1px_1px_rgb(0,0,0)]' >
          <span className='w-8/10 flex justify-center items-center ml-3'> GOAL</span>  <FaChevronDown className='w-2/10 text-xs mr-2' />
        </div>
      ...
      <div className='flex md:w-1/3 w-2/3 justify-between  items-center'>
        ...
        <div className='text-white mr-1 md:mr-3 flex items-center justify-between'>{totalAmt.toFixed(2)} USD   <span onClick={togglenavmenu} className='bg-[#417715] cursor-pointer rounded-full p-1 shadow-[0px_0px_5px_black] ml-2'><RiMenu3Fill className='size-4' /></span></div>
